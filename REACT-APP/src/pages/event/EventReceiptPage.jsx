...
const RECEIPT_INIT = [
  {
    ...
    deskripsi:    'DP 50% — Jasa Event Organizer Fun Run Jakarta 2026',
    lineItems: [
      { ket: 'Jasa Event Organizer & Koordinasi',   total: 35_000_000 },
      { ket: 'Instruktur & Tim Lapangan (5 orang)', total: 7_500_000  },
    ],
    subtotal:   42_500_000,
    ppn:        4_675_000,
    grandTotal: 47_175_000,
  },
  {
    noReceipt:    'RCP-EV-26-0002',
    orderId:      'EV-26-0002',
    namaKlien:    'Komunitas Sehat ID',
    jenis:        'Community Event',
    namaEvent:    'Yoga Festival Senayan 2026',
    jenisEvent:   'Yoga Festival',
    periode:      'Jul 2026',
    periodeFull:  'Juli 2026',
    total:        15_000_000,
    tglBayar:     '02 Jul 2026',
    tglBayarFull: '02 Juli 2026',
    metode:       'Transfer Bank (Mandiri)',
    metodeLabel:  'Transfer Bank',
    picKlien:     'Ibu Sari',
    program:      'Co-Organizer – Yoga Festival Senayan 2026',
    noRef:        'TRF-20260702-002',
    deskripsi:    'DP 50% — Jasa Co-Organizer Yoga Festival Senayan 2026',
    lineItems: [
      { ket: 'Jasa Event Organizer & Koordinasi', total: 12_000_000 },
      { ket: 'Instruktur Yoga & Pilates',          total: 3_000_000  },
    ],
    subtotal:   15_000_000,
    ppn:        1_650_000,
    ...
    <>
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wide mb-1.5">Corporate & Brand</p>
            <p className="text-[28px] font-bold text-blue-600 leading-none">{RECEIPT_INIT.filter(r => r.jenis === 'Corporate Event' || r.jenis === 'Brand Event').length}</p>
            <p className="text-[11px] text-text-muted mt-1.5">Corporate & Brand Event</p>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wide mb-1.5">Community & Gov</p>
            <p className="text-[28px] font-bold text-purple-600 leading-none">{RECEIPT_INIT.filter(r => r.jenis === 'Community Event' || r.jenis === 'Government Event').length}</p>
            <p className="text-[11px] text-text-muted mt-1.5">Community & Government</p>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wide mb-1.5">Private</p>
            <p className="text-[28px] font-bold text-pink-500 leading-none">{RECEIPT_INIT.filter(r => r.jenis === 'Private Event' || r.jenis === 'Family Gathering').length}</p>
            <p className="text-[11px] text-text-muted mt-1.5">Private & Family</p>
          </div>
        </div>
          ...
          <div className="p-3 bg-green-50 border border-green-200 rounded-xl flex items-center justify-between no-print">
            ...
            <button
              onClick={() => {
                ...
                navigate('/event/receipt', { replace: true })
              }}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="overflow-x-auto w-full">
            <table className="w-full text-sm" style={{ minWidth: '1000px' }}>
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {[['No Receipt',175],['Order ID',130],['Nama Klien',160],['Nama Event',160],['Periode',120],['Total',130],['Tanggal Bayar',120],['Metode',110],['Aksi',100]].map(([h, mw]) => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-gray-400">Tidak ada receipt yang cocok dengan filter.</td></tr>
                ) : filtered.map(r => (
                  <tr key={r.noReceipt} onClick={() => setPreview(r)} className="border-b border-gray-100 hover:bg-blue-50/30 transition-colors duration-150 cursor-pointer">
                    ...
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      <p className="text-xs font-medium text-gray-900">{r.namaKlien}</p>
                      {r.jenisEvent && <p className="text-[10px] text-gray-400">{r.jenisEvent}</p>}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-gray-700 whitespace-nowrap">{r.namaEvent || '—'}</td>
