...
function StatusBadge({ status }) {
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${EV_DOC_STATUS_CLS[status] ?? 'bg-gray-100 text-gray-500'}`}>
      <span className="w-1.5 h-1.5 rounded-full bg-current shrink-0" />
      {EV_DOC_STATUS_LABEL[status] ?? status}
    </span>
  )
}
      ...
      <div className="min-h-full flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-[520px] shadow-2xl" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <h3 className="text-[15px] font-bold text-text-primary">Detail Dokumen Event</h3>
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                ...
                <p className="text-[13px] font-semibold text-text-primary">{doc.klien}</p>
              </div>
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                <p className="text-[10.5px] font-bold text-text-muted uppercase tracking-wide mb-1">Jenis Dokumen</p>
                <p className="text-[13px] font-semibold text-text-primary">{doc.jenis}</p>
              </div>
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                <p className="text-[10.5px] font-bold text-text-muted uppercase tracking-wide mb-1">Tgl Upload</p>
                <p className="text-[13px] font-semibold text-text-primary">{doc.tglUpload}</p>
              </div>
              <div className="bg-gray-50 rounded-xl px-4 py-3">
                <p className="text-[10.5px] font-bold text-text-muted uppercase tracking-wide mb-1">Berlaku Hingga</p>
                <p className="text-[13px] font-semibold text-text-primary">{doc.berlakuHingga}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {STATUS_OPTS.map(s => (
                    <option key={s} value={s}>{EV_DOC_STATUS_LABEL[s]}</option>
                  ))}
                </select>
              </div>
            </div>
...
function UploadModal({ onClose, onSave }) {
  const [form, setForm]     = useState({ jenis: '', klien: '', berlaku: '', fileName: '', status: 'drafting' })
  ...
  function handleSubmit() {
    const errs = {}
    if (!form.jenis)  errs.jenis  = 'Wajib diisi'
    if (!form.klien)  errs.klien  = 'Wajib diisi'
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <h3 className="text-[15px] font-bold text-text-primary">Dokumen Baru Event</h3>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <select value={form.jenis} onChange={e => set('jenis', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#1E1C43]">
              <option value="">Pilih Jenis...</option>
              {JENIS_OPTS.map(j => <option key={j}>{j}</option>)}
            </select>
            {errors.jenis && <p className="text-[11px] text-accent mt-1">{errors.jenis}</p>}
          </div>
          <div>
            <label className="block text-[12px] font-semibold text-text-primary mb-1.5">Nama Klien <span className="text-accent">*</span></label>
            <select value={form.klien} onChange={e => set('klien', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#1E1C43]">
              <option value="">Pilih Klien...</option>
              {KLIEN_OPTS.map(k => <option key={k}>{k}</option>)}
            </select>
            {errors.klien && <p className="text-[11px] text-accent mt-1">{errors.klien}</p>}
          </div>
          <div>
            ...
            <select value={form.status} onChange={e => set('status', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#1E1C43]">
              {STATUS_OPTS.map(s => <option key={s} value={s}>{EV_DOC_STATUS_LABEL[s]}</option>)}
            </select>
          </div>
          <div>
...
export default function EventDocumentsPage() {
  const [docs, setDocs]             = useState(eventDocuments)
  ...
  const filtered = useMemo(() => docs.filter(d => {
    const matchTab    = activeTab === 'Semua' || d.jenis === activeTab
    const matchStatus = !fStatus || d.status === fStatus
    const matchSearch = !fSearch || d.klien.toLowerCase().includes(fSearch.toLowerCase())
  ...
  const draftCount  = docs.filter(d => d.status !== 'signed').length
  ...
  function handleNewDoc(form) {
    ...
    setDocs(prev => [{
      id:           `#EV-DOC-${String(nextId).padStart(3, '0')}`,
      klien:        form.klien,
      jenis:        form.jenis,
      tglUpload:    today,
      berlakuHingga: form.berlaku,
      status:       form.status,
      revisions,
      googleDocsUrl: '',
    }, ...prev])
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-[22px] font-bold text-text-primary">Pemberkasan Event</h1>
          <p className="text-sm text-text-muted mt-1">Kelola dokumen kontrak, MOU, dan LOI event</p>
        </div>
        <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold text-white bg-accent hover:bg-accent-hover transition-colors">
          ...
          return (
            <div key={s.label} className="bg-white rounded-2xl border-[1.5px] border-gray-200 p-5 flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-[22px] font-bold text-text-primary">{s.value}</p>
              </div>
            </div>
          )
        })}
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-white rounded-xl border-[1.5px] border-gray-200 p-1.5 w-fit">
        {TABS.map(tab => (
          <button key={tab} onClick={() => { setActiveTab(tab); setPage(1) }}
            className={`px-6 py-2 rounded-lg text-[13px] font-medium transition-colors ${activeTab === tab ? 'bg-[#1E1C43] text-white font-semibold' : 'text-text-muted hover:text-text-primary'}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border-[1.5px] border-gray-200 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr>
                {['ID Berkas', 'Nama Klien', 'Jenis', 'Tgl Upload', 'Berlaku Hingga', 'Status', 'Preview', 'Aksi'].map(h => (
              ...
              {pageRows.length === 0 ? (
                ...
                return (
                  <tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-text-muted whitespace-nowrap">{d.klien}</td>
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-gray-100 text-text-muted">{d.jenis}</span>
                    </td>
                    <td className="px-4 py-3 text-text-muted whitespace-nowrap">{d.tglUpload}</td>
                    <td className="px-4 py-3 text-text-muted whitespace-nowrap">{d.berlakuHingga}</td>
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setDetailDoc(d)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-text-muted hover:border-[#1E1C43] hover:text-[#1E1C43] transition-colors">
                        <Eye size={14} />
                      </button>
                    </td>
                  </tr>
