...
import { APT_LEADS_INIT, STAGE_CLS, PIC_EFM_OPTS } from '../../data/b2bData'
...
function AddModal({ onClose, onSave }) {
  ...
  const handleSave = () => {
    if (!form.nama.trim()) { alert('Nama apartemen wajib diisi.'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-[520px] shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <h3 className="text-[16px] font-bold text-text-primary">Tambah Lead Apartment</h3>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-[12px] font-semibold text-text-primary mb-1.5">Nama Apartemen <span className="text-accent">*</span></label>
            <input value={form.nama} onChange={set('nama')} placeholder="Apartemen Nama..."
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[12px] font-semibold text-text-primary mb-1.5">No HP</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.nilaiEst} onChange={set('nilaiEst')} placeholder="Rp 7jt/bln"
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:border-[#1E1C43] font-[Poppins]" />
            </div>
            <div>
      ...
      <div className="bg-white rounded-2xl w-full max-w-[520px] max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="bg-gray-50 rounded-xl px-4 py-3 border-l-[3px] border-[#27AE60]">
            ...
            <p className="text-[12px] text-text-muted mt-0.5">Apartment · PIC: {lead.pic}</p>
          </div>
          <div>
            ...
            <input value={form.nilaiKontrak} onChange={set('nilaiKontrak')} placeholder="Rp 7.000.000"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:border-[#1E1C43] font-[Poppins]" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
        ...
        <div className="flex justify-end gap-2.5 px-6 py-4 border-t border-gray-100">
          ...
          <button onClick={() => { onConvert(lead.id); onClose() }}
            ...
            <Check size={14} strokeWidth={2.5} />Konfirmasi — Jadikan Klien
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-[480px] shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <h3 className="text-[16px] font-bold text-text-primary">Detail Lead Apartment</h3>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-text-muted hover:border-text-primary transition-colors"><X size={15} /></button>
        </div>
        <div className="px-6 py-5">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2 bg-gray-50 rounded-xl px-4 py-3">
              <p className="text-[10.5px] font-bold text-text-muted uppercase tracking-wide mb-1">Nama Apartemen</p>
              <p className="text-[15px] font-bold text-text-primary">{lead.nama}</p>
            </div>
...
export default function B2BLeadsApartmentPage() {
  const [leads, setLeads]           = useState(APT_LEADS_INIT)
  ...
  const handleAddSave = (form) => {
    const newId = `BA-${String(leads.length + 1).padStart(3, '0')}`
  ...
  const showConvertHint = () => { setToast(true); setTimeout(() => setToast(false), 2500) }

  return (
    <div className="p-6 space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[22px] font-bold text-text-primary">Leads Apartment</h1>
          <p className="text-sm text-text-muted mt-1">Kelola prospek apartemen B2B — Klik ✓ untuk convert lead Closing menjadi Klien</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex flex-wrap items-center gap-2.5">
        <select value={fBulan} onChange={e => { setFBulan(e.target.value); setPage(1) }}
          className="h-9 px-3 rounded-lg border border-gray-200 text-[13px] bg-white focus:outline-none focus:ring-2 focus:ring-[#1E1C43]/20 font-[Poppins]">
        ...
        <button onClick={() => { setFBulan(''); setFTahun(''); setFStage(''); setFSearch('') }}
          className="h-9 px-4 rounded-lg bg-[#1E1C43] text-white text-[13px] font-semibold hover:bg-[#2D2B5A] transition-colors">Reset</button>
        <div className="flex items-center gap-2 flex-1 min-w-[200px] bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 focus-within:border-[#1E1C43] focus-within:bg-white transition-colors">
          ...
          <input type="text" value={fSearch} onChange={e => { setFSearch(e.target.value); setPage(1) }}
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {['ID', 'Nama Apartemen', 'PIC Klien', 'No HP', 'Est. Nilai Kontrak', 'Stage', 'Tanggal', 'Aksi'].map(h => (
                ...
                <tr key={lead.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      {lead.stage === 'Closing' ? (
                        <button title="Convert menjadi Klien"
                        ...
                        <button title="Hanya tersedia saat stage = Closing" onClick={showConvertHint}
      ...
      {modal === 'add' && <AddModal onClose={() => setModal(null)} onSave={handleAddSave} />}
